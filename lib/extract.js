...
var Extractor = (function () {
    function Extractor(options) {
        this.options = _.extend({
            ...
            markerNamesPlural: [],
            moduleName: ['gettextCatalog', 'this.translateService', 'translateService'],
            moduleMethodString: ['getString', 'instant'],
        ...
        var thisRef = this;
        this.attrRegex = [];
		this.options.startDelim.map(function(startDelim, index)  {
			thisRef.attrRegex.push(mkAttrRegex(startDelim, thisRef.options.endDelim[index] || thisRef.options.endDelim[0], thisRef.options.filterName));
		});
        ...
        function isGetString(node) {
            return node !== null &&
                ...
                node.callee.object !== null && (
                    -1 !== self.options.moduleName.indexOf(node.callee.object.name) || (
                        // also allow gettextCatalog calls on objects like this.gettextCatalog.getString()
                        node.callee.object.property &&
                        -1 !== self.options.moduleName.indexOf(node.callee.object.property.name))) &&
                node.callee.property !== null &&
                -1 !== self.options.moduleMethodString.indexOf(node.callee.property.name) &&
                node.arguments !== null &&
                node.arguments.length;
        }
